...
def updateCPAP(label):
    def update_CPAP():
        global dutyCycle
        #print(flowList[0])
        out = "PWM: " + str(dutyCycle)
        label.config(text = out)
        label.after(1000, update_CPAP)
    update_CPAP()
...
class mainFunc(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        for F in (StartPage, PageOne, PageTwo, PageThree, CPAP):
...
class PageOne(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        label.pack(pady=15,padx=(20, 0))

        button1 = tk.Button(self, text="CPAP", bg = "navy", fg = "grey90", font = font_Button,
                            command=lambda: controller.show_frame(CPAP))
        button1.place(height = 69, width = 214, x = 512, y = 90)

        button2 = tk.Button(self, text="", bg = "navy", fg = "grey90", font = font_Button,
                            command=lambda: controller.show_frame(PageTwo))
        button2.place(height = 69, width = 134, x = 462, y = 180)

        button3 = tk.Button(self, text="", bg = "navy", fg = "grey90", font = font_Button,
                            command=lambda: controller.show_frame(PageThree))
        button3.place(height = 69, width = 224, x = 417, y = 270)
        
        button4 = tk.Button(self, text="Back", relief = 'ridge', bg = "red4", fg = "grey90", font = font_Button,
...
class CPAP(tk.Frame):
    ...
    def __init__(self, parent, controller):
        tk.Frame.__init__(self, parent)
        label = tk.Label(self, text="CPAP", font=font_Button, bg = "grey30", fg = "white")
        label.pack(pady=10,padx=(40, 0))

        label1 = tk.Label(self, text="", font=font_Button, bg = "grey30", fg = "white")
        label1.place(x = 480, y = 250)
        #label1.pack(pady=10,padx=10)
        updateCPAP(label1)
        
        CPAP_Func(10)
    
        button1 = tk.Button(self, text="Increase Pressure", bg = "navy", fg = "grey90", font = font_Button,
                            command= lambda: CPAP_Func(10))
        button1.place(height = 69, width = 225, x = 568, y = 110)

        button2 = tk.Button(self, text="Decrease Pressure", bg = "navy", fg = "grey90", font = font_Button,
                            command= lambda: CPAP_Func(-10))
        button2.place(height = 69, width = 225, x = 200, y = 110)
